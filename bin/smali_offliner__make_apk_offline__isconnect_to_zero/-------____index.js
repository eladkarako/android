"use strict";
const DEBUG = true
     ,log   = function function(){if(false===DEBUG){return;} console.error.bind(console).apply(console,arguments); }
     ,fs    = require("fs")
     ;


async function action_per_file(path){
  const content = await fs.readFile(path,{encoding: "utf8"})
       ,matches = Array.from(content.matchAll(content_pattern))
       ;
  if(0 === matches.length){
    log("[INFO] not including pattern, skipping", "\r\n", path);
    return;
  }
